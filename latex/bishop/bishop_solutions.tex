...
\begin{equation*}
	KL(p || q) = - \int p(\vect{x}) 
	\ln \left( \frac{p(\vect{x})}{q(\vect{x})} \right)
	\, d \vect{x}
\end{equation*}
...
\begin{align*}
	...
	\vect{\Sigma}^{-T}.
\end{align*}
The first equation is differentiation of a quadratic form with respect to the vector, which is straightforward.
The second equation is more involved, and is from \emph{the matrix cookbook}\footnote{\url{https://www.math.uwaterloo.ca/~hwolkowi/matrixcookbook.pdf}}.
...
\end{equation*}
Taking constants out of the integrals and multiplying with $\vect{\Sigma}$ (which we assume to be invertible) yields $\vect{\mu} = \int p(\vect{x}) \vect{x} \, d \vect{x}$ as desired.

Next we differentiate with respect to  $\vect{\Sigma}$ and set it equal to zero, giving
\begin{equation*}
	\int p(\vect{x}) 
	\vect{\Sigma}^{-T}
	...
	\, d \vect{x} = \vect{0}.
\end{equation*}
Right-multiplying by $\vect{\Sigma}^{-T}$ and using $\vect{\Sigma}^{T} = \vect{\Sigma}$, we solve for $\vect{\Sigma}^{-1}$ and obtain
\begin{equation*}
	\vect{\Sigma}^{-1} = \int 
	p(\vect{x})  
	\left( \vect{x} - \vect{\mu} \right)
...
The expression for $b_N$ requires some more care.
By the linearly of the expected value
\begin{equation*}
b_N \simeq 
	\frac{1}{2}
	\left[
	...
	\right].
\end{equation*}
As $N \to \infty$, we see that $\E [\mu] = \bar{x}$ and (from Equation (1.50) in the book)
\begin{equation*}
...
\sum_{n=1}^{N} \left( x_n - \bar{x} \right)^2,
\end{equation*}
where the final asymptotic equality holds because $(\bar{x} - \mu_0)^2$ does not grow as $N \to \infty$.
Substituting these we see that the expected value of the variational posterior distribution over the precision asymptotically equals the sample variance, i.e.
\begin{equation*}
...
Starting from Equation (10.43)
\begin{equation*}
	\ln q^{\star}(\vect{Z})
	=
	\E_{\vect{\pi}, \vect{\mu}, \vect{\Lambda}} [
	\ln 
	p(X, Z, \pi, \mu, \Lambda)
	]
	+ \text{const}
\end{equation*}
we go to (10.44) by noticing that only $p(Z\mid \pi)$ and $p(X \mid Z, \mu, \Lambda)$ in (10.41) are functions of $Z$.
We have
\begin{equation*}
	\E_{\vect{\pi}} [
	\ln 
	...
	\ln
	\prod_{n=1}^{N}
	\prod_{k=1}^{K} \pi_k^{z_{nk}}
	 \right]
	 =
	 \E_{\vect{\pi}}
	 \left[ 
	 \sum_{n=1}^{N}
	 \sum_{k=1}^{K} z_{nk} \ln \pi_k
	 \right]
	 =
	 \sum_{n=1}^{N}
	 \sum_{k=1}^{K} z_{nk} 
	 ...
	 \right]
\end{equation*}
and since $\ln p(x_n, z_n \mid, \mu, \Lambda) \propto \ln \abs{\vect{\Lambda}_k} - D \ln (2 \pi) - \E \left[ (\vect{x}_n - \vect{\mu})^T \vect{\Lambda}_k (\vect{x}_n - \vect{\mu}) \right]$ we have
\begin{align*}
...
\sum_{k=1}^{K}
z_{nk}
 \left( \frac{1}{2} \E_{ \vect{\Lambda}} [\ln \abs{\vect{\Lambda}_k}]
	- 
	\frac{D}{2} \ln (2 \pi)
	-
	\frac{1}{2} \E_{\vect{\mu}, \vect{\Lambda}} 
	\left[ (\vect{x}_n - \vect{\mu})^T \vect{\Lambda}_k (\vect{x}_n - \vect{\mu}) \right] \right) 
\end{align*}
Combining the expressions for $\E_{\vect{\pi}} [ \ln  p(Z\mid \pi) ]$ and $\E_{\vect{\mu}, \vect{\Lambda}} [ \ln  p(X \mid Z, \mu, \Lambda)]$ yields Equation (10.46) in the book.
