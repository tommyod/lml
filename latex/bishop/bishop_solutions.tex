...
We split this problem into three parts: the expected value, variance and mode.
\begin{easylist}[enumerate]
	\ListProperties(Space=\listSpace, Space*=\listSpace, Numbers1=l, FinalMark1={)})
	# The expected value is computed from the definition as
	\begin{align*}
	...
	# We will employ $\var[\mu] = \E[\mu^2] - \E[\mu]^2$.
	Similarly to the previous sub-problem above, we observe that
	\begin{align*}
	\E[\mu^2] &= \frac{\Gamma(a + b)}{\Gamma(a ) \Gamma( b)} \int_0^a  \mu^{(a+2)-1} (1- \mu)^{b-1} \, d\mu \\
	&= \frac{\Gamma(a + b)}{\Gamma(a ) \Gamma( b)} \frac{\Gamma(a +2 ) \Gamma( b)}{\Gamma(a + b + 2)}
	= \frac{(a+1) a}{(a + b +1) (a+b)},
	\end{align*}
	where in the last equality we have used $\Gamma(x+2) = (x+1)x \Gamma(x)$, which is merely repeated application of $\Gamma(x+1) = x\Gamma(x)$ twice.
	
	At this point, we're left with
	\begin{equation*}
		\var[\mu] = \E[\mu^2] - \E[\mu]^2 = \frac{(a+1) a}{(a + b +1) (a+b)} - \left( \frac{a}{a + b} \right),
	\end{equation*}
	which after some algebra equals the desired result.
	
	# The mode is the maximum of the distribution.
...
To show Equation (2.270), we apply the definition of $E_x \left[ x \mid y \right]$ and $E_y [f(y)]$ to get
\begin{align*}
...
The second part, which consists of proving Equation (2.271), is slightly more involved.
It helps to keep track of what the quantities are, i.e. constants, functions of $x$ or functions of $y$.
It helps to know beforehand, from the definition of conditional variance, that
\begin{equation}
...
\end{equation}
This does not come as a surprise, since it's merely the equation $\var_x[ x ] = \E_x [x^2] - \E_x [x ]^2$ conditioned on $y$.

Let's examine the first term in the right hand side of (2.270) first, i.e. $\E_y \left[ \var_x[ x \mid y ]  \right]$.
...
We now investigate the second term in (2.270), i.e. 
