...
If a polynomial in $P(\omega_1)$ has two zeros, then it must at least be quadratic.
...
\end{align*}
The sample variance is proportional to the impurity estimate defined in subproblem b).
...
For each of these alignments, there will between $1$ and $m$ comparisons, depending on whether matches are found.
There is a probability $d^{-1}$ of a match for each comparison made.
The expected number of comparisons may be expressed as a \emph{arithmetico-geometric} sum, i.e. a sum of terms which are the product of an arithmetic and geometric sum.

First, observe that there are $(n - m + 1)$ alignments of $\vect{x}$ and \emph{text}.
For every alignment, $m$ characters may be compared at most.
Let $1 \leq  \ell \leq m$ be the number of comparisons made before the loop terminates.
The loop terminates when no match is found.
...
Let us study $P(\ell)$, the probability of $\ell$ matches before termination of the loop. We observe that
\begin{alignat*}{2}
	P(\ell = 1) &= 1 - \frac{1}{d} &\qquad (\text{no match}) \\
...
We take the expected value to obtain the expected number of comparisons, and introduce a variable $r = 1/d$ for convenience.
\begin{align*}
...
\end{align*}
Using the summation formula for a arithmetico-geometric sum\footnote{See wikipedia for the summation formula.} we obtain
\begin{align*}
\E \left[ \ell \right] = \sum_{k=1}^{m} P(\ell = k) k
& =\left(1 - r\right) \left( 1 + 2r + 3r^2 + \dots + m r^{m-1} \right) + m r^m \\
&=\left(1 - r\right) \left( 
...
\end{align*}
After some algebra on this final fraction, we obtain the desired result, which is
\begin{equation*}
	\E \left[ \ell \right] = \sum_{k=1}^{m} P(\ell = k) k = \frac{1 - r^m}{1 - r} = \frac{1 - d^{-m}}{1 - d^{-1}}.
\end{equation*}
The expected number of comparisons is therefore $(n - m + 1) \frac{1 - d^{-m}}{1 - d^{-1}}$.
The inequality $\frac{1 - d^{-m}}{1 - d^{-1}} \leq 2$ presented in the problem description stems from the fact that
\begin{equation*}
...
\textbf{A naive approach} of looping through the alphabet (size $d$) for every letter in $\vect{x}$ (size $m$) would give an $O(dm)$ algorithm.
...
\end{verbatim}
Below is actual Python code implementing the above.
\begin{Verbatim}
...
If $d\gg m$ or $m \gg d$, such an algorithm can avoid looping over the longest string, but we will not pursue this here.
\begin{easylist}[enumerate]
	\ListProperties(Space=\listSpace, Space*=\listSpace, Numbers1=l, FinalMark1={)})
	# The time complexity is $O(m + d)$.
	# The space complexity is $O(d)$ in the algorithm above. We can avoid storing the empty matches (for instance by using the \texttt{collections.defaultdict} data structure in Python) and get the storage down to $O(\min(m, d))$.
	# Using the algorithm above $d + m$ operations.
	Since $m < d$ here, we can use a \texttt{collections.defaultdict} data structure in Python and bring it down to $m$ operations.
\end{easylist}
Below is a print showing that \texttt{collections.defaultdict} leads to faster runtime.
\begin{Verbatim}
...
(ii) Reflexivity; 
(iii) Symmetry; and 
(iv) Triangle inequality. These properties are listed on page 187 in the book, which corresponds to section 4.6.1.
...
\begin{easylist}
	...
	## Triangle inequality is not always obeyed. A counterexample follows.
\end{easylist}
...
This provides a counterexample of the symmetry property of a metric.

\textbf{Counterexample for triangle inequality} 
Let $\epsilon$ be the empty string.
Consider the transformation shown in the diagram below.
If the cost of substitution is much greater than the cost of deletion and insertion, then $D(\mathtt{a}, \epsilon) + D(\epsilon, \mathtt{b}) < D(\mathtt{a}, \mathtt{b})$. 
