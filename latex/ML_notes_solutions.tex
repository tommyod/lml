...
# Bayes theorem is
\begin{equation*}
	P(\omega_j \mid \vect{x}) = \frac{p(\vect{x} \mid \omega_j) P(\omega_j)}{p(\vect{x})}
	= \frac{\text{likelihood} \times \text{prior}}{\text{evidence}}
\end{equation*}
and the Bayes decision rule is to choose $\omega$ such that
\begin{equation*}
	\omega = \arg \max_j P(\omega_j \mid \vect{x}).
\end{equation*}

# Risk functions with risks (losses) other than zero-one is possible.
...
# If features $y$ are missing, we integrate over it using the sum rule
\begin{equation*}
	p(x) = \int p(x, y) \, dy = \int p(x \mid y) p(y) \, dy.
\end{equation*}

# In Bayesian belief networks, influences are represented using a directed graph.
If $B$ is dependent on $A$, we add $A \to B$ to the network.
